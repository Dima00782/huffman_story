...
HuffmanTreeBuilder::HuffmanTreeBuilder(std::string_view text) {
  ...
  for (const auto& [symbol, frequence] : char_count) {
    nodes.push_back(std::make_unique<TreeNode>(std::string(1, symbol), frequence, nullptr, nullptr));
  }
  ...
  while (nodes.size() >= 2) {
    ...
    const auto total_frequency = first_minimum->frequency_ + second_minimum->frequency_;
    auto union_node = std::make_unique<TreeNode>("", total_frequency, std::move(first_minimum), std::move(second_minimum));
